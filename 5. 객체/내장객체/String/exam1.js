...
{
    // RegEX.property.test : 주어진 문자열에서 정규표현식과 일치하면 true 불일치하면 false.
    const str = '010-1234-5678';
    const re = /^010-\d{4}-\d{4}$/;
    // const re = new RegExp(`010-\\d{4}-\\d{4}`);
...
}
{
    ...
    const phone = '052-1234-5678';
    const result = phone.split('-');
    console.log(result);    // [ '052', '1234', '5678' ]
    if(result[0] == '052') {
        console.log('울산');
    }else {
        console.log('울산이외 지역');
    }
    console.log(result.join(''));   // 05212345678
    console.log(phone.replaceAll('-','')); // 05212345678
}
{
